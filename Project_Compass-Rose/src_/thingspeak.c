...
void send_data_to_thingspeak(uint16_t value_field1, uint16_t value_field2, uint8_t direction) {
    char request[512];
    snprintf(request, sizeof(request), "GET /update?api_key=%s&field1=%d&field2=%d&field3=%d HTTP/1.1\r\nHost: %s\r\nConnection: close\r\n\r\n",
             THINGSPEAK_API_KEY, value_field1, value_field2, direction, THINGSPEAK_URL);
